...
export function mountLab(host:HTMLElement,onLost:()=>void){
 ...
 const worlds=createWorlds(wake);
 ...
 renderer.domElement.addEventListener('pointermove',e=>{if(intro)return;if(down){if(Math.hypot(e.clientX-down.x,e.clientY-down.y)>5)down.moved=true;if(down.moved){yaw-=(e.clientX-down.lastX)*.006;pitch=T.MathUtils.clamp(pitch+(e.clientY-down.lastY)*.005,.15,1.35);orbit();wake();}down.lastX=e.clientX;down.lastY=e.clientY;return;}const o=hit(e);outline.visible=!!o;if(o)outline.box.setFromObject(o);renderer.domElement.style.cursor=o?'pointer':'grab';controls.onHover(o?.userData.label||'',e.clientX,e.clientY);wake();});
 renderer.domElement.addEventListener('pointerup',e=>{if(down&&!down.moved){const o=hit(e);if(o)dispatch(o.userData.action);}down=null;});renderer.domElement.addEventListener('pointercancel',()=>down=null);renderer.domElement.addEventListener('pointerleave',()=>{outline.visible=false;controls.onHover('',0,0);wake();});
 ...
 const dispose=()=>{if(dead)return;dead=true;cancelAnimationFrame(frame);resized.disconnect();worlds.dispose();renderer.dispose();motion.removeEventListener('change',changed);document.removeEventListener('visibilitychange',wake);};renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();dispose();onLost();});window.addEventListener('pagehide',e=>{if(!e.persisted)dispose();});
